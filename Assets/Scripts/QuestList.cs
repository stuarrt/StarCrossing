...
public class QuestList : MonoBehaviour {
	...
	void Start() {
		...
		showinfo = false;
		qinfo = "";
		quests.Add(new Quest("Put out the Fire", questDC ("Quest0Displayed"), questDC ("Quest0Completed"))); //0
		quests.Add(new Quest("Talk to Mayor Gran", questDC ("Quest1Displayed"), questDC ("Quest1Completed"))); //1
		quests.Add(new Quest("Talk to Ruby", questDC ("Quest2Displayed"), questDC ("Quest2Completed"))); //2
		quests.Add(new Quest("Check on the Town", questDC ("Quest3Displayed"), questDC ("Quest3Completed"))); //3
		quests.Add(new Quest("Talk to Burnadine", false, false)); //4
		quests.Add(new Quest("Talk to Dr. Cyabin", false, false)); //5
		quests.Add(new Quest("Talk to Hat Person", false, false)); //6
		quests.Add(new Quest("Talk to Ika", false, false)); //7
		quests.Add(new Quest("Talk to Khet", false, false)); //8
		quests.Add(new Quest("Talk to Morry", false, false)); //9
		quests.Add(new Quest("Talk to Sally", false, false)); //10
		quests.Add(new Quest("Talk to Trox", false, false)); //11
		quests.Add(new Quest("Find Wallace Poster", false, false)); //12
		quests.Add(new Quest("Find a Coconut", false, false)); //13
	}
	...
	void Update () {
		...
		if (quests[13].display && (Inventory.Instance.CurrentInventory [Inventory.InventoryObjects.Coconut01]) || 
			Inventory.Instance.CurrentInventory [Inventory.InventoryObjects.Coconut02]){
			quests[13].completed = true;
		}
		if (quests[12].display && Inventory.Instance.CurrentInventory [Inventory.InventoryObjects.Shovel]) {
		...
		if (notebook) {
			...
			if (Input.GetKeyDown(KeyCode.Alpha5) && (quests[12].display || quests[13].display)) {
				page = 4;
			}
			/*
	...
	void OnGUI() {
		...
		if (notebook) {
			...
			*/
			...
			if (page == 3 && (quests [12].display || quests [13].display)) {
				if (GUI.Button (new Rect (275, (Screen.height / 4) + 20, 20, 20), ">", buttonStyle)) {
					page += 1;
				}
			}
			...
			if (page == 0) {
				if (quests [0].display) {
					...
					if (GUI.Button (new Rect (20, (Screen.height / 4) - 70, 280, 25), quests [0].name + " - " + questCheck (quests [0].completed), qlist)) {
						showinfo = true;
						questimage = player;
						qinfo = "Your crash landing has started fires all throughout the town! Put out the fire using your water cannon by pressing and holding down the L key.";
					}
					if (!quests [0].display) {
				...
				if (page == 1) {
					print ("Should be displaying because: " + quests[3].display);
					if (quests [3].display) {
					//	print ("Should be displaying because: " + quests[3].display);
						//GUI.Box (new Rect (20, (Screen.height / 4) - 70, 280, 25), "", qboxStyle);
				...
				if (page == 4) {
					...
					if (quests [13].display) {
						//GUI.Box (new Rect (20, (Screen.height / 4) - 40, 280, 25), "", qboxStyle);
						//GUI.Label (new Rect (15, (Screen.height / 4) - 40, 270, 100), quests [13].name + " - " + questCheck(quests [13].completed), qlist);
						if (GUI.Button (new Rect (20, (Screen.height / 4) - 40, 280, 25), quests [13].name + " - " + questCheck (quests [13].completed), qlist)) {
							showinfo = true;
							questimage = fossil;
